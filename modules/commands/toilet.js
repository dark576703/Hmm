...
async function makeImage({ one, two }) {
    ...
    hon_img.resize(320, 345).composite(circleOne.resize(70, 70), 100, 200).composite(circleTwo.resize(70, 70), 100, 200);
...
module.exports.run = async function ({ event, api, args, Currencies }) { 
  ...
  if (!two) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: `বেশি বাল পাকলামির জন্য তোরে টয়লেটে ফেলে দিলাম। এবাল লে ঠেলা বুঝ গু খা বলদ 😁😁`, attachment: fs.createReadStream(path)}, threadID, () => fs.unlinkSync(path), messageID));
  }
      }
